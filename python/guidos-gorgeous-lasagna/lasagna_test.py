...
try:
    ...
    if "EXPECTED_BAKE_TIME" in message:
        ...
        raise ImportError(
            f"We can not find or import the constant {item_name} in your"
        ...
        # pylint: disable=raise-missing-from
        raise ImportError(
            "In your 'lasagna.py' file, we can not find or import the"
...
class LasagnaTest(unittest.TestCase):
    ...
    def test_elapsed_time_in_minutes(self):
        ...
        for variant, (layers, time, total_time) in enumerate(
            ...
            with self.subTest(
                f"variation #{variant}", layers=layers, time=time, total_time=total_time
            ):
                failure_msg = f"Expected {time} minutes elapsed, but the timing was calculated incorrectly."
                self.assertEqual(
                    elapsed_time_in_minutes(layers, time), total_time, msg=failure_msg
                )
